...
function createGameManager({ storage }) {
  ...
  function buildStateFor(room, viewerId) {
    ...
    return {
      ...
      aliveCounts: aliveFactionCount(room),
  ...
  function aliveFactionCount(room) {
    const alive = alivePlayers(room);
    return {
      mafia: alive.filter(player => player.faction === 'mafia').length,
      citizen: alive.filter(player => player.faction === 'citizen').length,
      cult: alive.filter(player => player.faction === 'cult').length
    };
  }
  ...
  function beginGame(room, options = {}) {
    const force = !!options.force;
    if (!force && !canStart(room)) throw new Error('At least 6 active players are required.');
    room.started = true;
    room.dayNumber = 0;
    room.winner = null;
    room.votes.clear();
    ...
    startTimer(room, 'reveal', room.settings.revealLength * 1000, () => {
      enterDay(room, 'The city wakes to a new investigation.', { advanceDay: true });
    });
  }

  function enterDay(room, message = 'Day discussion begins.', options = {}) {
    const advanceDay = options.advanceDay !== false;
    if (advanceDay) {
      room.dayNumber = (room.dayNumber || 0) + 1;
    } else {
      room.dayNumber = Math.max(1, room.dayNumber || 1);
    }
    room.votes.clear();
    room.actions.clear();
    ...
    addLog(room, message);
    const dayDuration = room.dayNumber === 1 ? 15 : 45;
    startTimer(room, 'day', dayDuration * 1000, () => enterVote(room));
  }
  ...
  function enterNight(room) {
    ...
    startTimer(room, 'night', 30 * 1000, () => resolveNight(room));
  }
  ...
  function enterResults(room, summary) {
    ...
    startTimer(room, 'results', 8000, () => {
      ...
      enterDay(room, `Dawn breaks on day ${room.dayNumber}.`, { advanceDay: true });
    });
  }
