...
function createGameManager({ storage }) {
  ...
  function maskPlayer(player, viewerId, room) {
    ...
    return {
      ...
      ready: player.ready,
      roleName: isSelf || room.phase === 'end' ? player.roleName : (player.revealed ? player.roleName : null),
      faction: isSelf || room.phase === 'end' ? player.faction : (player.revealed ? player.faction : null),
      team: player.team,
      statuses: {
  ...
  function allowChat(room, player, channel) {
    if (channel === 'public') return (room.phase === 'lobby' || room.phase === 'day') && player.alive && !player.spectator && room.settings.publicChatDuringDay;
  ...
  function quickPlay(socket, payload = {}) {
    ...
    return createRoom(socket, { ...payload, mode });
  }
