...
function createStorage(baseDir) {
  ...
  const cloudStateUrl = process.env.CLOUD_STATE_URL || '';
  const cloudStateToken = process.env.CLOUD_STATE_TOKEN || '';
  ...
  const load = () => {
    ...
    if (cloudStateUrl) {
      // Best-effort cloud sync for hosted deployments.
      // Uses built-in fetch so no extra dependency is required.
      globalThis.fetch(cloudStateUrl, {
        method: 'GET',
        headers: cloudStateToken ? { Authorization: `Bearer ${cloudStateToken}` } : {}
      })
        .then(res => res.ok ? res.json() : Promise.reject(new Error(`Cloud load failed (${res.status})`)))
        .then(parsed => {
          Object.assign(state, {
            users: parsed.users || [],
            sessions: parsed.sessions || [],
  ...
  const save = async () => {
    ...
    if (cloudStateUrl) {
      try {
        await globalThis.fetch(cloudStateUrl, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            ...(cloudStateToken ? { Authorization: `Bearer ${cloudStateToken}` } : {})
          },
          body: payload
        });
      } catch (error) {
        // Local save remains the source of truth if cloud backup fails.
      }
    }
  };
  ...
  return {
    async register({ username, password }) {
      ...
      const user = {
        ...
        isAdmin: cleanName.toLowerCase() === 'pranat02',
        level: 1,
        xp: 0,
        wins: 0,
        losses: 0,
    ...
    async login({ username, password }) {
      ...
      user.isAdmin = cleanName.toLowerCase() === 'pranat02' || user.isAdmin === true;
      const sessionToken = createSession(user.id);
