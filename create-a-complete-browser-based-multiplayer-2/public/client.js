(() => {
  ...
  const nodes = {
    ...
    roleHud: el('roleHud'),
    roleHudState: el('roleHudState'),
    roleHudBody: el('roleHudBody'),
  ...
  function renderRoom() {
    ...
    nodes.roomMeta.innerHTML = [
      ...
      `${badge(room.dayNumber <= 1 ? 'Day 1: 15s' : 'Day 2+: 45s')}`,
      `${badge('Night: 30s')}`
    ].join(' ');
    const readyBtn = el('readyBtn');
    if (readyBtn) readyBtn.hidden = true;
    const startBtn = el('startBtn');
    if (startBtn) startBtn.textContent = state.user?.isAdmin ? 'Force Start' : 'Start';
    nodes.playerList.innerHTML = state.roomState.players.map(player => `
      <article class="player-item">
        ...
        <div class="player-meta">
          <span>${escapeHtml(room.phase === 'lobby' ? 'Hidden' : (player.roleName || 'Hidden'))}</span>
  ...
  function renderRoleHud() {
    if (!state.roomState) return;
    const currentRoleName = state.roomState.viewerRole;
    const role = state.roles.find(r => r.name === currentRoleName) || null;
    const visible = !!currentRoleName && state.roomState.room.phase !== 'lobby';
    nodes.roleHudState.textContent = visible ? 'Active' : 'Hidden';
    nodes.roleHudBody.innerHTML = visible && role ? `
      <div class="role-card-big">
        <div class="room-top">
          <div>
            <h3 style="margin:0">${escapeHtml(role.name)}</h3>
    ...
    ` : `
      <div class="tiny">Your role will appear here once the game starts.</div>
    `;
    nodes.roleHud.classList.toggle('hidden', false);
  }
  ...
  function renderActions() {
    ...
    if (room.phase === 'lobby') {
      ...
      actions.push(actionButtonsForRole(viewerRole, room, state.roomState.aliveCounts || {}));
  ...
  function actionButtonsForRole(roleName, room, aliveCounts = {}) {
    ...
    const isMafia = role.faction === 'mafia';
    const loneMafiaCanKill = isMafia && (aliveCounts.mafia || 0) === 1;
    if (loneMafiaCanKill && !actions.includes('kill')) {
      actions.unshift('kill');
    }
    return `
      ...
      <div class="tiny">${loneMafiaCanKill ? 'You are the last mafia alive, so you can perform a kill.' : 'Select a target from the player list and then activate your action.'}</div>
    `;
  }
  ...
  function registerHandlers() {
    ...
    const readyBtn = el('readyBtn');
    if (readyBtn) readyBtn.addEventListener('click', () => socket.emit('setReady', { ready: true }));
    el('startBtn').addEventListener('click', () => socket.emit('roomCommand', { command: state.user?.isAdmin ? 'forceStart' : 'start' }));
